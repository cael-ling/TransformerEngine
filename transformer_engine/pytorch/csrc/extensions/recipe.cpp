...
void hadamard_transform_amax(const at::Tensor& tensor,
                             at::Tensor& rowwise_amax,
                             at::Tensor& columnwise_amax,
                             int64_t rht_matrix_random_sign_mask) {
  auto input_tensor = tensor.contiguous();
  const TensorWrapper& te_input = makeTransformerEngineTensor(input_tensor);

  TORCH_CHECK(rowwise_amax.scalar_type() == at::kFloat,
              "rowwise_amax must be a float tensor");
  TORCH_CHECK(rowwise_amax.numel() == 1,
              "rowwise_amax must have exactly one element");
  TORCH_CHECK(columnwise_amax.scalar_type() == at::kFloat,
              "columnwise_amax must be a float tensor");
  TORCH_CHECK(columnwise_amax.numel() == 1,
              "columnwise_amax must have exactly one element");

  // Mirror NVFP4Quantizer: empty NVFP4_1D_SCALING with two amax slots.
  TensorWrapper te_output(NVTE_NVFP4_1D_SCALING);
  te_output.set_amax(rowwise_amax.data_ptr<float>(), DType::kFloat32,
                     std::vector<size_t>{1});
