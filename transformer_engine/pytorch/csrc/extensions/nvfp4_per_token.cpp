...
void assemble_per_token_tensors(const at::Tensor& input,
                             at::Tensor q_row, at::Tensor s_dec_row, at::Tensor row_amax,
                             at::Tensor q_col, at::Tensor s_dec_col, at::Tensor col_amax,
                             bool rowwise, bool columnwise, int mode,
  ...
  if (rowwise) {
    ...
    TORCH_CHECK(row_amax.numel() == M,
                "row_amax numel mismatch: expected M=", M, ", got ", row_amax.numel());
    ...
    if (needs_fp4_outputs) {
      ...
      TORCH_CHECK(q_row.scalar_type() == at::ScalarType::Byte,
                  "q_row must be uint8 (FP4 packed)");
      TORCH_CHECK(s_dec_row.scalar_type() == at::ScalarType::Byte,
                  "s_dec_row must be uint8 (FP8 e4m3 raw bytes)");
      TORCH_CHECK(q_row.numel() == M * K / 2,
                  "q_row numel mismatch: expected M*K/2=", M * K / 2, ", got ", q_row.numel());
      TORCH_CHECK(s_dec_row.numel() == M * K / 16,
                  "s_dec_row numel mismatch: expected M*K/16=", M * K / 16,
                  ", got ", s_dec_row.numel());
  ...
  if (columnwise) {
    ...
    TORCH_CHECK(col_amax.numel() == K,
                "col_amax numel mismatch: expected K=", K, ", got ", col_amax.numel());
    ...
    if (needs_fp4_outputs) {
      ...
      TORCH_CHECK(q_col.scalar_type() == at::ScalarType::Byte,
                  "q_col must be uint8 (FP4 packed)");
      TORCH_CHECK(s_dec_col.scalar_type() == at::ScalarType::Byte,
                  "s_dec_col must be uint8 (FP8 e4m3 raw bytes)");
      TORCH_CHECK(q_col.numel() == K * M / 2,
                  "q_col numel mismatch: expected K*M/2=", K * M / 2, ", got ", q_col.numel());
      TORCH_CHECK(s_dec_col.numel() == K * M / 16,
                  "s_dec_col numel mismatch: expected K*M/16=", K * M / 16,
                  ", got ", s_dec_col.numel());
...
void nvfp4_per_token_quantize(const at::Tensor& input,
                              at::Tensor q_row, at::Tensor s_dec_row, at::Tensor row_amax,
                              at::Tensor q_col, at::Tensor s_dec_col, at::Tensor col_amax,
                              bool rowwise, bool columnwise,
                              bool with_rht, int64_t random_sign_mask_t) {
  TensorWrapper in_te;
  TensorWrapper out_te(NVTE_NVFP4_1D_SCALING);
  assemble_per_token_tensors(input, q_row, s_dec_row, row_amax,
                          q_col, s_dec_col, col_amax,
                          rowwise, columnwise, /*mode=*/0, in_te, out_te);
  const auto stream = at::cuda::getCurrentCUDAStream();
  nvte_nvfp4_per_token_quantize(
      in_te.data(), nullptr, out_te.data(),
      with_rht ? 1 : 0,
...
void nvfp4_per_token_amax(const at::Tensor& input, at::Tensor row_amax,
  ...
  TensorWrapper in_te;
  TensorWrapper out_te(NVTE_NVFP4_1D_SCALING);
  assemble_per_token_tensors(input, empty_u8, empty_u8, row_amax,
                          empty_u8, empty_u8, col_amax,
                          rowwise, columnwise, /*mode=*/1, in_te, out_te);
  const auto stream = at::cuda::getCurrentCUDAStream();
  // C-API matches prod's `int` convention; only low 16 bits are consumed.
  nvte_nvfp4_per_token_amax(
      in_te.data(), nullptr, out_te.data(),
      with_rht ? 1 : 0,
      static_cast<int>(random_sign_mask_t & 0xFFFF),
      stream);
...
void nvfp4_per_token_encode(const at::Tensor& input,
                                   at::Tensor q_row, at::Tensor s_dec_row, at::Tensor row_amax,
                                   at::Tensor q_col, at::Tensor s_dec_col, at::Tensor col_amax,
                                   bool rowwise, bool columnwise,
                                   bool with_rht, int64_t random_sign_mask_t) {
  ...
  assemble_per_token_tensors(input, q_row, s_dec_row, row_amax,
                          q_col, s_dec_col, col_amax,
                          rowwise, columnwise, /*mode=*/2, in_te, out_te);
  const auto stream = at::cuda::getCurrentCUDAStream();
...
void nvfp4_per_token_post_scale(at::Tensor d, const at::Tensor &row_amax_a,
  ...
  TORCH_CHECK(row_amax_a.numel() == M,
              "row_amax_a numel mismatch: expected M=", M, ", got ", row_amax_a.numel());
  TORCH_CHECK(row_amax_b.numel() == N,
              "row_amax_b numel mismatch: expected N=", N, ", got ", row_amax_b.numel());

  const auto stream = at::cuda::getCurrentCUDAStream();

  TensorWrapper d_te = makeTransformerEngineTensor(
      d.data_ptr(),
      std::vector<size_t>{static_cast<size_t>(M), static_cast<size_t>(N)}, DType::kBFloat16);
...
void nvfp4_per_token_gemm(const at::Tensor &a_data, const at::Tensor &b_data,
                          ...
                          const at::Tensor &a_row_amax, const at::Tensor &b_row_amax,
                          at::Tensor d, const at::Tensor &workspace, int64_t m, int64_t n,
  ...
  TORCH_CHECK(a_data.dim() == 2 && b_data.dim() == 2 && d.dim() == 2,
              "a_data/b_data/d must be 2D");
  TORCH_CHECK(a_data.size(0) == m && a_data.size(1) * 2 == k,
              "a_data shape mismatch: expected (M=", m, ", K/2=", k / 2, "), got (",
              a_data.size(0), ", ", a_data.size(1), ")");
  TORCH_CHECK(b_data.size(0) == n && b_data.size(1) * 2 == k,
              "b_data shape mismatch: expected (N=", n, ", K/2=", k / 2, "), got (",
  ...
  TORCH_CHECK(a_row_amax.numel() == m,
              "a_row_amax numel mismatch: expected M=", m, ", got ", a_row_amax.numel());
  TORCH_CHECK(b_row_amax.numel() == n,
              "b_row_amax numel mismatch: expected N=", n, ", got ", b_row_amax.numel());
...
void nvfp4_per_tensor_gemm(const at::Tensor &a_data, const at::Tensor &b_data,
                           const at::Tensor &a_sf, const at::Tensor &b_sf,
                           const at::Tensor &a_amax, const at::Tensor &b_amax,
                           at::Tensor d, const at::Tensor &workspace, int64_t m, int64_t n,
                           int64_t k, double alpha, double beta) {
  TORCH_CHECK(a_data.is_cuda() && b_data.is_cuda() && a_sf.is_cuda() && b_sf.is_cuda() &&
...
void build_per_token_output_wrapper(
    TensorWrapper& out_te, int64_t M_i, int64_t K, bool rowwise, bool columnwise,
    const at::Tensor& q_row, const at::Tensor& s_dec_row, const at::Tensor& row_amax,
  ...
  if (rowwise) {
    ...
    out_te.set_rowwise_data(
        q_row.data_ptr(), DType::kFloat4E2M1,
...
void nvfp4_per_token_group_quantize(
    ...
    bool rowwise, bool columnwise,
    bool with_rht, int64_t random_sign_mask_t) {
  TORCH_CHECK(rowwise || columnwise,
              "At least one of rowwise/columnwise must be True.");
  ...
  for (size_t i = 0; i < num_tensors; ++i) {
    ...
    TORCH_CHECK(split_sections[i] % 64 == 0, "split_sections[", i,
  ...
  TensorWrapper in_te = makeTransformerEngineTensor(
      input.data_ptr(),
      std::vector<size_t>{static_cast<size_t>(sum_M), static_cast<size_t>(K)}, in_dtype);
  ...
  for (size_t i = 0; i < num_tensors; ++i) {
    ...
    build_per_token_output_wrapper(
        wrappers.back(), M_i, K, rowwise, columnwise,
        rowwise ? q_row_list[i] : empty_dummy,
        rowwise ? s_dec_row_list[i] : empty_dummy,
        rowwise ? row_amax_list[i] : empty_dummy,
        columnwise ? q_col_list[i] : empty_dummy,
        columnwise ? s_dec_col_list[i] : empty_dummy,
        columnwise ? col_amax_list[i] : empty_dummy);
    handles.push_back(wrappers.back().data());
  }

  nvte_group_nvfp4_per_token_quantize(in_te.data(), handles.data(),
                                      split_sections_sz.data(), num_tensors, rowwise,
                                      columnwise,
                                      static_cast<int>(with_rht),
...
void nvfp4_per_token_group_amax(const at::Tensor& input,
  ...
  nvte_group_nvfp4_per_token_amax(in_te.data(), handles.data(), split_sections_sz.data(),
                                  num_tensors, rowwise, columnwise,
                                  static_cast<int>(with_rht),
                                  static_cast<int>(random_sign_mask_t),
                                  stream);
}
...
nvfp4_per_token_group_quantize_bulk(const at::Tensor& input,
  ...
  TORCH_CHECK(input.dim() == 2,
              "nvfp4_per_token_group_quantize expects a 2D input, got ", input.dim(), "D");
  TORCH_CHECK(input.scalar_type() == at::ScalarType::BFloat16,
  ...
  TORCH_CHECK(K % kPerTokenTile == 0,
              "Per-token grouped kernel requires K % ", kPerTokenTile, " == 0; got K=", K);

  const size_t num_tensors = split_sections.size();
  TORCH_CHECK(num_tensors > 0, "split_sections must not be empty");
  TORCH_CHECK(num_tensors <= 64,
              "num_tensors must be <= 64 (kernel arg-struct cap); got ", num_tensors);
  ...
  TORCH_CHECK(acc == sum_M, "sum(split_sections) = ", acc,
              " must equal input.size(0) = ", sum_M);
  ...
  for (size_t i = 0; i < num_tensors; ++i) {
    ...
    if (columnwise) {
      ...
      auto s_dec_col_flat =
          s_dec_col_bulk.narrow(0, K * m_off / kBlockK, K * M_i / kBlockK);
  ...
  TensorWrapper in_te = makeTransformerEngineTensor(
      input.data_ptr(),
      std::vector<size_t>{static_cast<size_t>(sum_M), static_cast<size_t>(K)},
      DType::kBFloat16);
  ...
  for (size_t i = 0; i < num_tensors; ++i) {
    ...
    build_per_token_output_wrapper(
        wrappers.back(), M_i, K, rowwise, columnwise,
        rowwise ? q_row_list[i] : empty_dummy,
        rowwise ? s_dec_row_u8_list[i] : empty_dummy,
        rowwise ? row_amax_list[i] : empty_dummy,
        columnwise ? q_col_list[i] : empty_dummy,
        columnwise ? s_dec_col_u8_list[i] : empty_dummy,
        columnwise ? col_amax_list[i] : empty_dummy);
    handles.push_back(wrappers.back().data());
  }
