...
#SBATCH --output=/lustre/fsw/portfolios/coreai/users/caell/github/caell/TransformerEngine/_scratch_mcore/slurm_logs/nvfp4-moe-%j.out
#SBATCH --error=/lustre/fsw/portfolios/coreai/users/caell/github/caell/TransformerEngine/_scratch_mcore/slurm_logs/nvfp4-moe-%j.err

set -euo pipefail

# --- Host-side config (paths are the LUSTRE host paths; inside the container
#     they appear under /home/caell via --container-mounts) -------------------
# IMAGE: base te_main image by default. After building the per-token image with
# sbatch_build_te_image.sh, point here and set SKIP_BUILD=1, e.g.:
#   sbatch --export=ALL,IMAGE=/lustre/.../image/arch100a_pt_2508_te_pertoken.sqsh,SKIP_BUILD=1 \
#          sbatch_moe_nvfp4_singlegpu.sh pertoken
IMAGE="${IMAGE:-/lustre/fsw/portfolios/coreai/users/caell/image/arch100a_pt_2508_te_main_9ad2e7b.sqsh}"
HOST_MOUNT="/lustre/fsw/portfolios/coreai/users/caell:/home/caell"
TE_DIR="/home/caell/github/caell/TransformerEngine"             # in-container path
SCRATCH_DIR="${TE_DIR}/_scratch_mcore"
RUN_SCRIPT="${SCRATCH_DIR}/run_moe_nvfp4_singlegpu.sh"
...
# Rolling cleanup: keep LATEST ckpt always + every-Nth milestone; previous
# non-milestone ckpt auto-deleted after the next save (~63G each). Must be a
# multiple of SAVE_INTERVAL. Prevents the 100T quota from filling up.
export SAVE_RETAIN_INTERVAL="${SAVE_RETAIN_INTERVAL:-10000}"
# Save+exit after this many TRAINING minutes so the job finishes under the 2h
# wall and a chained (dependent) job can resume. 110 leaves ~10 min headroom for
# container startup + final save. Set 0 to disable. See submit_chain.sh.
...
# Quoted heredoc ('EOF'): the body is passed verbatim; $VARS expand inside the
# container against the inherited (exported) environment.
...
#    The image ships TE main (no per-token); the per-token code lives in the
#    mounted workspace. A fresh container's site-packages is ephemeral, so we
#    must re-run the editable install. build/ persists on the mount -> this is
#    an incremental no-op relink when nothing changed (~1-2 min), full build on
#    first run after a source change.
if [[ "${SKIP_BUILD:-0}" != "1" ]]; then
    echo "[job] (re)installing editable TE from $TE_DIR ..."
    cd "$TE_DIR"
    NVTE_CUDA_ARCHS=100a NVTE_BUILD_THREADS_PER_JOB=8 \
...
# Sanity: TE must import AND expose a per-token symbol (proves the workspace
# build, not the image's main TE, is active).
python - <<'PY'
...
# Additive RHT/SR semantics for BOTH paths: a run's +rht/+sr suffixes mean those
# features are ON; absence means OFF.
#   - pertoken: RHT/SR default OFF -> opt IN via NVTE_NVFP4_PER_TOKEN_RHT/SR=1
#   - prod    : RHT/SR default ON in TE -> we DISABLE the unselected ones via
#               NVTE_NVFP4_DISABLE_RHT / NVTE_NVFP4_DISABLE_STOCHASTIC_ROUNDING.
# Net effect (uniform): bare `prod`/`pertensor` = block-scaling only (no RHT/SR);
# production-default per-tensor = `pertensor+rht+sr`. 2D stays at TE default (on).
...
#   run_dir_for <mode> [sr] [rht] [oned] [twod]  ->  work/<run name>  (stable, no timestamp)
run_dir_for () { echo "${WORK_ROOT}/$(exp_name "$1" "${2:-}" "${3:-}" "${4:-}" "${5:-}" "${6:-}")${RUN_TAG:+-${RUN_TAG}}"; }
...
run_one () {
    ...
    local run_name; run_name="$(exp_name "$mode" "$sr" "$rht" "$oned" "$twod" "$fb")${RUN_TAG:+-${RUN_TAG}}"
