...
#
# Adapted from the 8-GPU EP=8 Ling-mini-v2 sbatch script down to ONE GPU:
#   - EP/TP/PP/CP all = 1 (no expert/tensor/pipeline/context parallel)
#   - MoE block based on Ling-mini-v2 (topk8 / 8 groups / group_topk4 /
#     moe-ffn 512 / shared 512 / hidden 2048 / ffn 5120 / seq 4096)
#   - num-layers 20 (1 dense + 19 MoE), full Ling-mini-v2 depth: fits one GB200
#     now that num-experts is reduced to 64 (expert MLP params dominate memory)
#   - num-experts reduced 256 -> 64: on a single GPU EP=1 puts ALL experts
#     local, and TE's grouped-NVFP4 kernels (RHT amax / cast-fusion) cap at 64
#     tensors per launch (arg-struct <4KB). Real training shards experts via
#     EP>1 so local experts <=64 and the cap never bites; here we cap to 64 so
#     all three modes (bf16/prod/pertoken) are apples-to-apples on one GPU.
#   - real OLMo-1124 data + Moonlight-16B tokenizer (DATA_MODE=mock to fall back)
#   - keeps the MoE essentials: grouped GEMM, shared expert, grouped routing
#     (num_groups/group_topk), sigmoid router w/ expert bias, alltoall dispatch
#
...
#     bash run_moe_nvfp4_singlegpu.sh prod        # production NVFP4 (block-scaling)
#     bash run_moe_nvfp4_singlegpu.sh pertoken    # NVFP4 per-token recipe
#
# Identical model / data / seed across modes; only the GEMM precision changes.
# ============================================================================
set -euo pipefail
...
if [[ "$MODE" == "pertoken" ]]; then
    ...
    # Per-token needs the norm fwd UNFUSED path (amax computed inside K1); the
    # default fused norm+amax path rejects per-token at quantizer.cpp.
    # NVTE_NORM_FWD_USE_CUDNN=1 forces impl=UNFUSED.
    export NVTE_NORM_FWD_USE_CUDNN=1
    echo "[run] MODE=pertoken  -> NVTE_NVFP4_PER_TOKEN=1 NVTE_NORM_FWD_USE_CUDNN=1"
    # Optional per-token RHT / SR (env-driven recipe knobs):
    #   export NVTE_NVFP4_PER_TOKEN_RHT=1
    #   export NVTE_NVFP4_PER_TOKEN_SR=1
elif [[ "$MODE" == "prod" ]]; then
    unset NVTE_NVFP4_PER_TOKEN || true
    # prod RHT/SR are ON by default in TE; here they are ablatable. Default to
    # OFF (block-scaling only) unless the caller enables them by setting the
    # DISABLE env to 0 (the sbatch wrapper does this for +rht / +sr specs).
    # Production-default per-tensor = both enabled (DISABLE_*=0). 2D stays on.
...
# so this script works whether it sits inside Megatron-LM/ or one level up in
# _scratch_mcore/ (override with MLM_DIR=...).
if [[ -n "${MLM_DIR:-}" ]]; then
    :
...
# ---------------------------------------------------------------------------
# 1b. Weights & Biases (key + project hardcoded; scalars need --tensorboard-dir).
# ---------------------------------------------------------------------------
export WANDB_API_KEY="a2fbd748ab147ba4a1c454f194c1ca0d2c59e888"
WANDB_PROJECT="NVFP4 per-token recipe MoE"
if [[ "$MODE" == "pertoken" ]]; then
    ...
    # Route A: 2D weight quant for the per-token recipe (NVTE_NVFP4_PER_TOKEN_WEIGHT_2D=1).
    ...
    # +1d ablation: 2D weight quant disabled -> 1D weights (mirrors wrapper -1d).
...
# Stable per-variant working dir (NO timestamp layer) so checkpoints save/load
# from the SAME path every launch -> resume just works per variant.
# work/<RUN_NAME>/{wandb,tb,checkpoints}.
WORK_ROOT="${WORK_ROOT:-${SCRIPT_DIR}/work}"
RUN_NAME="${RUN_NAME:-${WANDB_EXP_NAME}${RUN_TAG:+-${RUN_TAG}}}"
# Keep the wandb run name identical to the (wrapper-provided or derived) dir
# name so variants like -1d / -rht / -sr never collide on wandb. The wrapper
# encodes the full variant in RUN_NAME; the local WANDB_EXP_NAME logic above
# only covers standalone runs.
...
LOG_ARGS=(
    ...
    #     plus every-Nth "milestone"; the previous non-milestone ckpt is deleted
    #     (async, only AFTER the new one lands) so disk never accumulates every
    #     save. Must be a multiple of SAVE_INTERVAL (mcore asserts this). Each
    #     ckpt here is ~63G, so unbounded saving filled the 100T quota -> failures.
    --save "$CKPT_DIR"
    --load "$CKPT_DIR"
...
# ---------------------------------------------------------------------------
# 2. Model: Ling-mini-v2 MoE block. All GEMM dims %128 (per-token alignment):
...
# NOTE: Ling-mini-v2 also sets --moe-shared-expert-overlap (overlaps shared-expert
# compute with dispatch comm). Omitted here: on single GPU (EP=1) there is no
# dispatch comm to overlap, and it depends on --overlap-grad-reduce/-param-gather
# which need >1 rank. It is perf-only and does not change numerics. Re-add if you
# move to multi-GPU.

# ---------------------------------------------------------------------------
# 3. Precision recipe (the part under test).
...
# ---------------------------------------------------------------------------
# 4. Parallelism: single GPU. EP/TP/PP/CP=1. No sequence-parallel (needs TP>1),
...
# ---------------------------------------------------------------------------
# 5. Data / tokenizer.  DATA_MODE=real (default) uses the downloaded OLMo-1124
...
DATA_PATH="${DATA_PATH:-/home/caell/github/caell/TransformerEngine/_scratch_mcore/datasets/olmo-1124/algebraic_stack_text_document}"
...
TOKENIZER_MODEL="${TOKENIZER_MODEL:-/home/caell/github/caell/TransformerEngine/_scratch_mcore/tokenizers/Moonlight-16B-A3B}"
...
# ---------------------------------------------------------------------------
# 6. Schedule: gbs/mbs = 8 microbatches/step. Steps via TRAIN_ITERS (def 20000).
# ---------------------------------------------------------------------------
